...
The current cryptographic scheme used by BEEFY is `ecdsa`. This is **different** from other keys being used in Substrate. Other
Substrate related cryptographic keys are using the `sr25519` scheme. The most noticeable difference is that an `ecdsa` public key 
